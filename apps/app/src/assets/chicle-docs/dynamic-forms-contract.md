...
The schema must include `schemaVersion` so the API can migrate old definitions without changing every consumer at
once.
...
| `service` | One approved service owns the write | Submit calls a published dynamic service such as `create_client_profile` or `update_client_profile`. |
...
```json
{
  ...
  "serviceKey": "update_client_profile",
  "operation": "update",
  "payloadMap": {
    "id": "{{input.client_id}}",
...
2. Generate published companion Dynamic Services for the table: `create_*`, `list_*`, and when a primary column exists
   `update_*` and `delete_*`.
...
```json
{
  ...
  "request": {
    ...
    "tableName": "custom_test",
    "columns": [
      { "name": "name", "type": "string", "length": 160, "nullable": true },
...
lookup service (`list_roles`) when the related table is visible. This keeps generated CRUD forms usable without
...
```json
{
  "persistence": {
    ...
    "defaultTarget": {
      "type": "dynamic_service",
      "serviceKey": "create_custom_clients"
    }
  },
  "actions": [
    {
      ...
      "onSuccess": [
        { "type": "show_message", "tone": "success", "message": "Record saved successfully." }
      ],
      "onError": [
        { "type": "show_message", "tone": "danger", "message": "The record could not be saved." }
      ]
    }
  ]
}
  ...
  "actions": [
    {
      ...
      "onError": [
        { "type": "show_message", "tone": "danger", "message": "Invalid credentials" }
      ]
    }
  ]
}
...
```json
{
  ...
  "confirm": {
    "message": "Validate this email now?"
  },
  "action": {
    "type": "execute_service",
  ...
  "feedback": {
    ...
    "errorMessage": "Email could not be validated."
  }
}
```
...
- Role resource policies restrict specific form keys the same way services and flows are restricted.
...
1. **Data**: key, title, description, category and owner.
2. **Design**: steps, fields, layout, validation, data sources and actions.
3. **Version**: create immutable version from the draft.
4. **Test**: run test fixtures in web, tablet and mobile preview.
5. **Publish**: expose the version to runtime, screens, menus and actions.
...
| `ProcessStepsComponent` | Lifecycle steps: Data, Design, Version, Test, Publish | Ready |
...
- Next cleanup: extract `StepManagerComponent`, `SchemaFieldEditorComponent`, `JsonEditorPanelComponent` and
  `VersionLifecyclePanelComponent` so Screens, Forms and builders share the same controls.
