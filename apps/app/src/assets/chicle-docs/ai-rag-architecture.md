...
RAG is the versioned knowledge layer for Chicle AI. The model is not expected to know Chicle from training. Chicle
retrieves the right project, template and tenant context before asking the model to generate or explain configuration.

The goal is to let assistants create safe declarative drafts for Dynamic Services, Dynamic Forms, Flows, Actions and
Screens without fine-tuning a model and without letting the model invent tables, fields, permissions or runtime
contracts.
...
used by people, Codex, tests and Chicle AI assistants.
...
templates are allowed only when the user has explicitly confirmed an action (`create draft`, `apply`, `continue`) or
when the provider fails; provider failures must be labeled as `Fallback mode` so the user knows it was not full model
reasoning.

Interactive turns use a short model budget. The local model should receive compact prompts and small output limits for
analysis; if it does not answer before `ai.fastTimeoutMs`, Chicle continues in `Light mode` using guided authoring,
catalog metadata and examples. The long `ai.timeoutMs` remains reserved for background generation, indexing or
...
The backend must treat short answers as stateful decisions, not as isolated prompts. Examples such as `roles only`,
`user + roles`, `by name`, `by email` or `user.id -> user_roles.userId` close the matching pending question. If
...
with a continuation such as "continue", "do it" or "generate the draft". This prevents the assistant from creating a generic
service just because a fast deterministic generator recognized one table.

Example:

```txt
User: I need a service that queries a user and, if it exists, goes to roles...
AI: I need to know which field arrives from the front. Suggestions: by email, by name, by id.
User clicks: by email
AI: advanced service with queryMode=multi_table, users -> user_roles -> roles. Suggestions: create advanced service, adjust input, explain join.
User clicks: create advanced service
AI: emits apply_dynamic_service_json for query_user_roles_by_email.
```

This small-step approach keeps local models from timing out and makes RAG safer because each turn has a narrower
question.
...
- `embedding_json` persistence or vector store.
...
- Raw customer records unless an explicit permission model allows it.
- Full logs.
