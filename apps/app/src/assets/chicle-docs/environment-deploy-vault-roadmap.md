...
Chicle treats deployment as an administrable contract, not as scattered files owned by each developer.

The server needs only bootstrap values that cannot safely live inside the database:

- `CHICLE_ENV`: selected environment profile such as `prod`.
- `CHICLE_VAULT_PATH`: secure mounted path where Chicle Vault stores the local master key.
- `CHICLE_SECRET_MASTER_KEY`: optional external master key when the server injects it securely.
- Database bootstrap values: host, port, database, user and password.

Everything else should be configured through the Environment And Deploy Center when possible:

- public Admin URL;
- public API URL;
- CORS origins;
- Swagger policy;
- AI provider URL and model;
- worker settings;
...
The deployment bundle generated by Admin is the reviewed output for a server:

- environment template for bootstrap values;
- Docker Compose profile for the selected environment;
- runtime config for public frontend values;
- service registry preview;
- secret checklist;
- healthcheck checklist;
- security validation report.

The owner should not need to remember which file controls each value. Admin shows one coherent experience, while the
bundle remains inspectable for operations and audit.

## Microservice Extraction Strategy

Chicle starts with a modular API and extracts a module only when the boundary is stable and the operational reason is
clear.

Good extraction reasons:

- independent horizontal scaling;
- heavy load that can affect the rest of the API;
- independent deployment cadence;
- strict isolation or compliance boundary;
- external SLA or ownership boundary;
- long-running work better owned by a worker or service.

Bad extraction reasons:

- splitting code only because a module exists;
- bypassing tenant, RBAC, audit or backend validation;
- sharing the same database tables directly from multiple services;
- hardcoding URLs inside forms, flows, services or screens.

Every extracted service must be represented in the Service Registry per environment. Dynamic objects use logical
service keys and operations, never fixed URLs.

Minimum service registry contract:
...
```json
{
  ...
  "type": "microservice",
  "baseUrl": "https://billing.company.internal",
  "healthPath": "/health",
  "authMode": "service_token",
  "secretRef": "secret:prod.microservice.billing.SERVICE_TOKEN",
  "timeoutMs": 8000,
...
Synchronous communication is used only when the caller needs an immediate answer. Slow, retryable or failure-tolerant
work must use events, outbox, jobs and workers.

Allowed communication paths:

- Admin or business app calls the API with user context.
- API resolves service keys through the Service Registry.
- Runtime executes dynamic services, forms and flows using logical operations.
- Microservices receive tenant context and authenticated service identity.
- Events and jobs coordinate long-running or resilient work.

Forbidden communication paths:

- frontend calling private microservices directly with secrets;
- dynamic JSON storing plain secrets;
- forms, flows or services storing hardcoded production URLs;
- one service writing another service's tables directly;
- extracted services skipping audit, tenant scope or idempotency.

This keeps the same dynamic contracts valid when a module lives inside the API, inside a worker or as a separated
microservice.
...
- Backend module: `EnvironmentDeployModule`.
- API base path: `/api/environment-deploy`.
- Default profiles seeded per tenant during setup and on first use: `local`, `dev`, `qa`, `pre`, `prod`.
- Admin page: `/environments`.
- Database migration:
  - `environment_profiles`;
  - `environment_variables`;
  - `environment_secrets`;
  - `service_registry`.
- Local Chicle Vault provider:
  - AES-256-GCM encryption;
  - master key outside DB at `CHICLE_VAULT_PATH`;
  - optional `CHICLE_SECRET_MASTER_KEY` bootstrap override;
  - masked secret reads.
- Environment variables for non-secret values.
- Environment secrets for sensitive values.
- Service Registry entries by environment.
- Runtime config endpoint.
- Deployment bundle preview:
  - compose template;
  - env template;
  - runtime config JSON;
  - deployment checklist.
- Readiness validation for production risks.
- Audit events for profile, variable, secret, registry and bundle operations.
- Permissions and menu seeds:
  - `env.read`;
  - `env.update`;
  - `secrets.write`;
  - `secrets.rotate`;
...
- Manual vault key backup procedure.
- Owner/admin Admin UI for environments and secrets.
- Permissions: `env.read`, `env.update`, `secrets.write`, `secrets.rotate`, `deploy.validate`, `deploy.generate`,
  `service_registry.manage`.
...
```txt
GET   /api/environment-deploy/overview
GET   /api/environment-deploy/environments
POST  /api/environment-deploy/environments
GET   /api/environment-deploy/environments/:key
PATCH /api/environment-deploy/environments/:key
POST  /api/environment-deploy/environments/:key/variables
POST  /api/environment-deploy/environments/:key/secrets
POST  /api/environment-deploy/environments/:key/service-registry
GET   /api/environment-deploy/environments/:key/runtime-config
GET   /api/environment-deploy/environments/:key/validate
GET   /api/environment-deploy/environments/:key/deployment-bundle
```
...
```json
{
  ...
  "description": "Public API URL used by the deployed frontend runtime config."
}
```
...
```json
{
  ...
  "scopeKey": "default",
  "key": "JWT_SECRET",
  "value": "replace-with-secret-from-owner",
  "status": "active",
  "description": "JWT signing secret for production.",
  "reauthPassword": "owner-current-password"
}
```
...
```txt
module inside API
  -> same dynamic contract
  -> same logical operation
  -> registry entry can point to internal API handler

module extracted as microservice
  -> same dynamic contract
  -> same logical operation
  -> registry entry points to service base URL and auth policy
```
