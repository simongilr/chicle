...
} from "../components/declarative-component.types";
import { DeclarativeActionRuntimeService } from "../components/declarative-action-runtime.service";
...
export type RuntimeAction =
  DeclarativeComponentAction | { type: string; [key: string]: unknown };
...
export class ActionRunnerService {
  ...
  private readonly runtime = inject(DeclarativeActionRuntimeService);
  ...
  async execute(
    ...
    const logId = this.runtime.begin(resolvedAction, context);
    try {
      const result = await this.executeResolved(resolvedAction, context);
      this.runtime.complete(logId, result);
      return result;
    } catch (error) {
      this.runtime.fail(logId, error);
      throw error;
    }
  }
  ...
  private async executeResolved(
    ...
    switch (resolvedAction.type) {
      case "navigate": {
        const target =
          this.stringValue(resolvedAction["to"]) ||
          this.stringValue(resolvedAction["route"]);
        if (!target) {
          return {
            handled: false,
            reason: "navigate requires to or route",
            action: resolvedAction,
          };
        }
      ...
      case "execute_service": {
        ...
        if (!serviceKey) {
          return {
            handled: false,
            reason: "execute_service requires serviceKey",
            action: resolvedAction,
          };
        }
      ...
      case "submit_form":
        return {
          handled: true,
          type: "submit_form",
          formKey: this.stringValue(resolvedAction["formKey"]) || null,
        };
      case "open_modal":
      case "show_modal":
        return {
          ...
          type: "open_modal",
          modalKey:
            this.stringValue(resolvedAction["modalKey"]) ||
            this.stringValue(resolvedAction["templateKey"]) ||
            null,
          action: resolvedAction,
        };
      case "show_message":
        return {
          ...
          message:
            this.stringValue(resolvedAction["message"]) ||
            this.stringValue(resolvedAction["text"]) ||
            "Action completed.",
        };
      case "set_state": {
        const key =
          this.stringValue(resolvedAction["key"]) ||
          this.stringValue(resolvedAction["path"]);
        if (!key) {
          return {
            handled: false,
            reason: "set_state requires key or path",
            action: resolvedAction,
          };
        }
      ...
      case "refresh_data":
        return {
          ...
          type: "refresh_data",
          target:
            this.stringValue(resolvedAction["target"]) ||
            this.stringValue(resolvedAction["sourceKey"]) ||
            "default",
        };
      case "queue_offline": {
        const queued = this.runtime.enqueue(
          this.stringValue(resolvedAction["queueKey"]) || "default",
          resolvedAction,
          context,
          this.payloadFor(resolvedAction, context),
        );
      ...
      case "emit_event":
        return {
          ...
          type: "emit_event",
          eventKey:
            this.stringValue(resolvedAction["eventKey"]) ||
            this.stringValue(resolvedAction["name"]) ||
            "event",
      ...
      case "create_record":
        return {
          handled: false,
          reason: "create_record handler pending",
          action: resolvedAction,
          context,
        };
      default:
        return {
          handled: false,
          reason: `Unknown action ${resolvedAction.type}`,
        };
    }
  }
  ...
  private payloadFor(
    ...
    if (Object.keys(payloadMap).length) {
      return this.bindings.resolveDeep(payloadMap, context) as Record<
        string,
        unknown
      >;
    }
    const input =
      this.asRecord(action["input"]) ?? this.asRecord(action["context"]) ?? {};
    if (Object.keys(input).length) {
      return this.bindings.resolveDeep(input, context) as Record<
        string,
        unknown
      >;
    }
