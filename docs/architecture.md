...
- Admin owns everything administrable, designable and configurable: security, tenants, preferences, components,
  services, forms, flows, DB designer, docs and Chicle AI.
- Business apps are real web, mobile and desktop apps. They are not visual components; they execute the business
  logic published by Admin through the API and are delivered as deployable artifacts.
...
- `docs/ui-presentation-architecture.md` defines adaptive PrimeNG/Ionic rendering and visual kit adapters.
...
`DynamicFlowClientService` mirrors the dynamic-service client with `available()` and `execute(key, input)`. A screen
builder can therefore bind a component to a published flow key without generating a new Angular service. Declarative
actions use `execute_flow` plus `flowKey` and `payloadMap`, so forms and screen components consume a published flow
without adding Angular API code.
...
The worker contract supports local execution and dedicated worker processes. Multiple API replicas coordinate through
database claims without changing flow definitions.

Flow Lifecycle closes the operational authoring loop:
...
- REST request/response mapping, SOAP, WebSocket and connector implementations;
...
Files are stored through `StorageService`. Local disk, MinIO/S3 and compatible drivers keep the same app contract.
