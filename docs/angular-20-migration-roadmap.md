...
application builder are separate future decisions. They must not be added to this
migration.
...
must be split into later behavior-preserving work rather than hidden inside the
version migration.
...
5. Update the operational manual and component documentation with final versions.
