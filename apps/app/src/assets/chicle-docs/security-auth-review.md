...
- Permission names as stable strings such as `users.list` and `roles.edit`.
...
- Password policy:
  ...
  - Add breached/weak password screening through the security policy adapter.
- Login response must not leak whether email or password failed.
- Login rate limiting is required before production. Basic persistent rate limiting is implemented; broader account risk controls use the same policy and audit model.
...
- Refresh token rotation is implemented. Reuse detection currently invalidates the session on mismatch; broader risk alerts use the same policy and audit model.
...
- Docker-packaged deployments may terminate TLS at a reverse proxy.
...
- Use Argon2id immediately or keep bcrypt for compatibility and migrate through an audited password-hash policy.
